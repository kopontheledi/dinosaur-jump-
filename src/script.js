...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateScore() {
    ...
    // Check if the score is 5 or higher
    if (score >= 5) {
      // Display the "Yeei, you rock" text
      const rockText = document.createElement("div");
      rockText.textContent = "Yeei, you rock";
      rockText.classList.add("rock-text"); // Add a CSS class for styling
      gameContainer.appendChild(rockText);

      // Set a timeout to remove the text after 2 seconds
      setTimeout(() => {
        gameContainer.removeChild(rockText);
      }, 2000);
    }
  }
